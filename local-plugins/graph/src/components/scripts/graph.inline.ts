    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      function renderLinks() {
        for (var i = 0; i < linkRenderData.length; i++) {
          ...
          if (hoveredNodeId !== null) {
            ...
            color = linkData.active ? gray : lightgray;
          }
      ...
      function renderLabels() {
        ...
        for (var i = 0; i < nodeRenderData.length; i++) {
          ...
          // Labels are always visible; hovered/current nodes just get a slightly
          // larger label so they still stand out.
          nodeData.label.alpha = 1;
          if (hoveredNodeId === nodeData.simulationData.id || isCurrent) {
      ...
      if (enableZoom) {
        var zoomed = function (event) {
          ...
          // Labels stay visible at all zoom levels; renderLabels() owns their alpha.
        };
      ...
      function animate() {
        ...
        for (var i = 0; i < nodeRenderData.length; i++) {
          ...
          if (x != null && y != null) {
            ...
            if (n.label) {
              n.label.position.set(x + width / 2, y + height / 2 - n.radius - 4);
            }
          }
        }
