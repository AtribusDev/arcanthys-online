...
// Allows optional whitespace around the `|` metadata separator, e.g.
// `[!infobox | right wikipedia]` as well as `[!infobox|right wikipedia]`.
// Without the `\s*` on either side, a space before the pipe causes the pipe
// itself (and the leading space) to be swallowed into the metadata capture
// group instead of being treated as a separator, leaking a stray `|` into
// the rendered `data-callout-metadata` attribute.
const calloutRegex = new RegExp(/^\[!([\w-]+)\s*\|?\s*(.+?)?\]([+-]?)/);
          ...
          return (tree: Root, _file) => {
            visit(tree, "blockquote", (node) => {
              ...
              if (match && match.input) {
                const [calloutDirective, typeString, rawCalloutMetaData, collapseChar] = match;
                const calloutMetaData = rawCalloutMetaData?.trim();
